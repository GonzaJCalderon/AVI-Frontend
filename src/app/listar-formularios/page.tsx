...
import { estados, departments as departamentos } from '@/utils/constants'
...
export default function ListarFormulariosPage() {
  ...
  useEffect(() => {
    const obtenerDatos = async () => {
      try {
        const res = await fetch('http://localhost:8000/api/listar_formularios.php')
        if (!res.ok) throw new Error('No se pudo obtener los formularios')
